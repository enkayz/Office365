...
export async function diagnosePurview(client: Client) {
    const report: any = {
    ...
    const addCheck = (name: string, status: 'PASS' | 'FAIL' | 'WARN', message: string, points = 1) => {
        report.checks.push({ name, status, message });
        report.maxScore += points;
        if (status === 'PASS') report.score += points;
    };
    ...
    try {
        ...
        const hasSublabels = labels.some((l: any) => l.parent);
        if (hasSublabels) {
        ...
        labels.forEach((l: any) => {
            ...
            if (scopes.includes('File') || scopes.includes('Email')) hasFileScope = true;
            if (scopes.includes('Site') || scopes.includes('UnifiedGroup')) hasSiteScope = true;
        });
        ...
        try {
            ...
            if (policies.length > 0) {
                ...
                const hasMandatory = policies.some((p: any) => p.isMandatory);
                if (hasMandatory) {
                ...
                // Check: Default Label
                const hasDefault = policies.some((p: any) => p.defaultLabelId);
                if (hasDefault) {
