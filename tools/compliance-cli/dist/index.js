...
program
    ...
    console.log({ tenantObjectId: org.id, displayName: org.displayName, verifiedDomains: org.verifiedDomains?.map((d) => d.name) });
});
program
...
program
    ...
    // eslint-disable-next-line no-console
    console.table(users.map((u) => ({ id: u.id, name: u.name, upn: u.upn, skuCount: u.skus.length })));
});
program
    .command('roles-list')
    ...
    const filtered = opts.filter ? roles.filter((r) => (r.displayName || '').toLowerCase().includes(String(opts.filter).toLowerCase())) : roles;
    // eslint-disable-next-line no-console
...
program
    ...
    const tenantGuid = (orgRes.value && orgRes.value[0]?.id);
...
program
    ...
    // Roles snapshot
    const roles = await listDirectoryRoles(client);
    const roleNames = new Set(roles.map((r) => r.displayName));
    const roleMissing = PURVIEW_RELEVANT_ROLES.filter(r => !roleNames.has(r));
    // Labels availability
    let labelsOk = true;
    try {
        await listSensitivityLabels(client);
    }
    catch {
        labelsOk = false;
    }
    ...
    let auditOk = true;
    try {
        await listAuditSubscriptions(credential, tenantGuid);
    }
    catch {
        auditOk = false;
    }
...
program
    ...
    roles.forEach((r) => { if (r.displayName)
        roleMap.set(r.displayName, { id: r.id, displayName: r.displayName }); });
    ...
    for (const name of present) {
        ...
        if (opts.members) {
            try {
                members = await listRoleMembers(client, id);
            }
            catch {
                members = [];
            }
...
program
    ...
    const contentItems = Array.isArray(contentAny) ? contentAny : (contentAny?.value || contentAny || []);
    ...
    const workers = Array.from({ length: Math.min(cc, items.length) }, async () => {
        while (queue.length) {
            ...
            if (!item)
                break;
            try {
                const events = await fetchAuditBlob(credential, item.contentUri);
                if (opts.stdout) {
