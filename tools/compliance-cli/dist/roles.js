export async function listDirectoryRoles(client) {
    const rolesRes = await client.api('/directoryRoles').get();
    const roles = rolesRes.value || [];
    return roles.map((r) => ({ id: r.id, roleTemplateId: r.roleTemplateId, displayName: r.displayName }));
}
export async function listRoleMembers(client, roleId) {
    ...
    return members.map((m) => ({ id: m.id, name: m.displayName, upn: m.userPrincipalName }));
}
